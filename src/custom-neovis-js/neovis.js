...
export default class NeoVis {
	...
	async buildNodeVisObject(neo4jNode) {
		...
		node.value = 20;
		// console.log('node label and value ', node.value, node.label);
			...
			.subscribe({
				...
				onCompleted: async () => {
					...
					if (this._network && this._network.body.data.nodes.length > 0) {
								...
								scaling: {
									...
									customScalingFunction: function (min,max,total,value) {
										if (max === min) {
											...
											let scale = 1 / (max - min);
											let result = Math.max(0,(value - min)*scale);
						...
						this._consoleLog('data nodes and edges');
						this._consoleLog(this._nodes);
						this._consoleLog(this._edges);
